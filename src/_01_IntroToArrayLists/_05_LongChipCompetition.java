...
c = lcc.theBeatles.get(i).getChips();
			for (int j = 0; j < c.size(); j++) {
				if (length <(c.get(j).getLength())) {
					length=c.get(j).getLength();
...
System.out.println("The longest chip competition winner is " + name + " with " +length + " size chip");
